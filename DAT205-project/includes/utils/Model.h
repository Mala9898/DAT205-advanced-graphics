...
/**
 * CREDIT https://learnopengl.com/Model-Loading/Model
...
public:
    ...
    void Draw(Shader &shader) {
        ...
        for(auto i = 0; i < textures.size(); i++) {
            ...
            glActiveTexture(GL_TEXTURE0 + i+1);                                  // activate texture unit before binding
            string number;
            ...
            glUniform1i(glGetUniformLocation(shader.ID, (name+number).c_str()), i+1); // tell sampler which texture unit
            glBindTexture(GL_TEXTURE_2D, textures[i].id);             // bind texture to texture unit
        }
